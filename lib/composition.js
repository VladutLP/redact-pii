...
function composeChildRedactors(opts = {}) {
    ...
    for (const regexpName of Object.keys(simpleRegexpBuiltIns)) {
        if (!opts.builtInRedactors ||
            ...
            const replaceWith = (opts.builtInRedactors && opts.builtInRedactors[regexpName] && opts.builtInRedactors[regexpName].replaceWith) || opts.globalReplaceWith;
            childRedactors.push(new SimpleRegexpRedactor_1.SimpleRegexpRedactor({
                regexpPattern: simpleRegexpBuiltIns[regexpName],
                replaceWith: replaceWith || lodash_1.snakeCase(regexpName).toUpperCase()
            }));
        }
    }
    if (!opts.builtInRedactors || !opts.builtInRedactors.names || opts.builtInRedactors.names.enabled !== false) {
        const replaceWith = (opts.builtInRedactors && opts.builtInRedactors.names && opts.builtInRedactors.names.replaceWith) || opts.globalReplaceWith;
        childRedactors.push(new NameRedactor_1.NameRedactor(replaceWith));
    }
